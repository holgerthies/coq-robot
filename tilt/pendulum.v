...
suff poly_fact : a * (x ^+ 2) + b * x + c = a * (x - x1) * (x - x2).
  move: xroot; rewrite poly_fact => /eqP; rewrite mulf_eq0 => /orP [].
    by rewrite mulrI_eq0; [apply/lregP|rewrite subr_eq0 => /eqP->; left].
...
rewrite /x1 /x2; case: (lerP 0 dlt) => [dltge0|dltlt0].
  ...
  rewrite (_ : 4 = 2 * 2); first by rewrite -natrM.
  by field.
move: xroot; have -> : a * (x ^+ 2) + b * x + c =
  a * ((x + b / (2 * a)) ^+ 2) + (c - b ^+ 2 / (4%:R * a)).
  rewrite [in X in _ = X + _]expr2.
  rewrite -mulrA mulrDr mulrDl [c - _]addrC addrA !mulrDr.
...
have := ane0; rewrite neq_lt => /orP [alt0|agt0]; last first.
  ...
  rewrite subr_gt0 ltr_pdivrMr; first by rewrite pmulr_rgt0.
  by rewrite mulrC -subr_lt0.
rewrite -oppr_eq0 opprD; apply: lt0r_neq0; rewrite ltr_wpDl //.
  by rewrite oppr_ge0 nmulr_rle0 // sqr_ge0.
rewrite oppr_gt0 subr_lt0 ltr_ndivlMr; first by rewrite mulrC nmulr_rlt0.
by rewrite mulrC -subr_lt0.
Qed.
...
apply: subr0_eq; rewrite -deriveB // /derive cvg_at_rightE.
...
Proof.
by apply: preimage_closed; [move=> ??; apply: V_continuous|apply: closed_le].
Qed.
...
have K1bnd : \forall M \near +oo, forall p, K p -> `| p..[1] | < M.
  ...
  rewrite -sqrtr_sqr ltr_sqrt // mulrAC -ltr_pdivrMl // invf_div.
    by rewrite mulr0 /B/=.
...
rewrite -[X in X + _](@mulfVK _ ((M%:num + m%:num * (p..[3] ^+ 2)) * ke%:num));
  first by rewrite lt0r_neq0 // pmulr_rgt0 // Mp_ms_gt0.
rewrite mulrC -mulrDr normrM pmulr_rgt0.
...
rewrite ler_pM // lef_pV2 ?posrE //; first by rewrite pmulr_rgt0 // Mp_ms_gt0.
...
rewrite subr0 !mxE /= -circp -![_ *: _]/(_ * _) invrM ?unitfE //.
  by rewrite circp.
...
rewrite -!mul2r mul1r mulrDr.
rewrite [2^-1 * _]mulrCA.
rewrite [2^-1 * (m%:num * _ * _)]mulrCA.
rewrite [2^-1 * _]mulrA mulVf// mul1r.
rewrite [2^-1 * _]mulrA mulVf// mul1r.
rewrite [_ / _]mulrC.
...
rewrite derive1E derive_val mulrI_eq0; first exact/lregN/lregP.
...
move=> limSKp tge0.
rewrite [RHS](_ : _ =
    - (kd%:num * (sol p t)..[1] + kv%:num * (Fpendulum (sol p t))..[1])).
...
case: (eqVneq ((sol p s)..[3]) 0) => [sol3e0|sol3ne0].
  ...
  rewrite -(expr1n R 2) => /eqP; rewrite eqf_sqr=> /orP [] /eqP->.
    by exists 1.
  by exists 0.
move=> /eqP; rewrite mulrI_eq0; first exact/lregP.
...
suff : (Fpendulum (sol p 0))..[4] = 0.
  rewrite Fpendulum4E // sol0 => /eqP; rewrite mulrI_eq0; first exact/lregP.
  by move/eqP.
